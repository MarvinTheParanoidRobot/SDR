...
CONST char* DEFAULT_DRAWING_FILENAME="tags_example.g";

// Timer interval (msec) to refresh the display.
...
{
   GlgSessionC glg_session( GlgFalse, InitAppContext, argc, argv );
   GlgViewer glg_viewer;
   ...
   for( int skip = 1; skip < argc; ++skip )
   {
      if( strcmp( argv[ skip ], "-random-data" ) == 0 )
      {
         ...
         glg_viewer.RANDOM_DATA = GlgTrue;
      ...
      else if( strcmp( argv[ skip ], "-live-data" ) == 0 )
      {
         ...
         glg_viewer.RANDOM_DATA = GlgFalse;
   ...
   */
   ...
   {
      GlgError( GLG_INFO, 
                (char *) "Drawing file is not supplied on command line, using default drawing tags_example.g." );
   ...
   */
   SetSize( glg_viewer, 0, 0, 800, 700 );

   // Setting the window title. */
   glg_viewer.SetResource( "ScreenName", "GLG Simple Viewer Example" );

   // Initialize glg control.
   glg_viewer.Init();
   
   // Add DataFeed to perform data acquisition.
   glg_viewer.AddDataFeed();

   // Set screen refresh interval.
   glg_viewer.UpdateInterval = UPDATE_INTERVAL;
