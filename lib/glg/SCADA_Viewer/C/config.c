                             ...
                             GlgLong * num_read_records )
{
   ...
   if( !menu_config_file )
   {
      GlgError( GLG_USER_ERROR, "Can't open configuration file" );
   ...
   while( 1 )
   {
      if( !fgets( row, 5001, menu_config_file ) )
      {
         if( !feof( menu_config_file ) )    /* Read error if not EOF. */
            GlgError( GLG_USER_ERROR, "Error reading configuration file." );
         break;
      }
	...
	sscanf( row, "%1001[^,],%1001[^,],%1001[^,],%1001[^,]", 
      ...
      if( num_read != 4 )
      {
	 GlgError( GLG_USER_ERROR, "Syntax error reading configuration file" );
...
#define SIZE_INCREMENT   5
   ...
   if( old_size < requested_size )
   {
      ...
      */
      ...
      /* Allocate new array */
      menu_array_new = GlgAlloc( sizeof( MenuRecord ) * new_size );
...
{   
   ...
   for( i=0; i<length; ++i )
   {
      if( !escape_sequence )
      {
         ...
         {
            escape_sequence = GlgTrue;   /* Start of a 2-char escape sequence */
            ++from_ptr;
            continue;
         }
      }
