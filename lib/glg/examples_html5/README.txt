The following GLG JavaScript examples are provided in the following 
subdirectories:

Dashboard
   Demonstrates how to animate a Glg drawing containing a panel (dashboard) of
   GLG controls and handle user interaction in a GLG widget, such as a button or
   a slider.
...
SimpleViewer
   Provides a sample implementation of a simple GLG viewer application that uses
   the GLG tags feature in a generic way, allowing to animate any GLG drawing
   created in the GLG Builder or GLG HMI Configurator.

GlgViewer
   Provides a sample implementation of a generic GLG viewer application that
   loads a GLG drawing created in the GLG Builder or GLG HMI Configurator,
   animates the drawing using tags defined in the drawing and handles user
   interaction, including commands attached to objects in the editor.
...
The examples use the Community Edition version of the GLG library (GlgCE.js) and
supporting files listed below. To use the examples with the production version,
the GlgCE.js and GlgToolkitCE.js files have to be replaced with the production
version of the files. Refer to the Using the JavaScript Version of the Toolkit
chapter of the GLG Programming Reference Manual for more information.

The GLG JavaScript library is deployed in the form of two files: GlgCE.js and
GlgToolkitCE.js. The library also uses the gunzip.min.js file for uncompressing
GLG drawings saved in the compressed format. These three script files must be
included in the html file before the application script as follows:

    <script defer src="GlgCE.js"></script>
    <script defer src="GlgToolkitCE.js"></script>
