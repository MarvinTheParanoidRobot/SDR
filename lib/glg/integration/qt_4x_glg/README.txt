...
functionality in a QT application. The widget loads a GLG drawing
...
The GLG drawing was created using the GLG Builder and contains samples
of the available GLG dials and gauges.

USING GLG C OR C++ API TO ACCESS OBJECTS IN THE DRAWING

The example uses the GLG C API to access objects inside the GLG drawing. 
...
Alternatively, the GLG C++ API can be used, in which case the type of the 
