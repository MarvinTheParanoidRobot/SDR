...
void QGlgWidget::SetViewport( GlgObject viewport_p ) 
{
   ...
   x11_info = x11Info();
   win_id = (Window) winId();
   display = x11_info.display();
   x_screen = ScreenOfDisplay( display, x11_info.screen() );
...
void QGlgWidget::ResizeViewport( int width, int height, int draw )
{
   ...
   if( !displayed )
   {
     ...
     GlgUpdate( viewport );  /* Update to redraw after a size change. */
}
