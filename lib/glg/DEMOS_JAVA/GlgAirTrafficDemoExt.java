...
//
// This is an extended version of the demo that uses an asynchronous map 
// loading request API to load a map in a separate thread: the program will
// continue to operate with the old zoom or pan state and will switch to 
// the new zoom or pan when the new map is ready. 
//
// The -no_request command-line option may be used to disable the use of
// a map loading request, in which case the demo will behave the same way as
// the regular non-extended version of the demo.
//
...
//////////////////////////////////////////////////////////////////////////
public class GlgAirTrafficDemoExt extends GlgJBean
  implements ActionListener, GlgGISRequestObserver
{
   //////////////////////////////////////////////////////////////////////////
   // The main demo class
   //////////////////////////////////////////////////////////////////////////
   ...
   /* Set to true to use asynchronous map requests to load the new map:
      the program will continue to operate with the old zoom or pan state
      and will switch to the new zoom or pan when the new map is ready.      

      If set to false, the map will change right away, and the application 
      behavior will depend on the ASYNC_MAP setting.
   */
   boolean USE_REQUEST = true;

   /* Controls map loading behavior when asynchronous map requests are not used.
   ...
   */
   ...
   GlgPoint
     NewExtent = new GlgPoint(),
     NewCenter = new GlgPoint();

   int
     UpdateInterval = 50,  // Update interval in msec.
     NumPlanes = 100,      // Number of airplanes to use in the demo.
     NumNodes,
     NumTrajectoryPoints,
     PlaneType = 0,
     NodeType = 0,
   ...
   boolean
     /* Always false for initial map loading, may be changed after that. */
     UseRequest = false,                               
     InitialLoad = true,
     MapLoaded = false,
     OrthoOnly = false,
   ...
   */
   ...
   // -map_server_url       - specify MapServerURL, default: use GIS object's 
   //                         MapServerURL property 
   ...
   // -use_request          - Load map via request (see USE_REQUEST comments)
   // -no_request           - Load map directly (see USE_REQUEST comments)
   //////////////////////////////////////////////////////////////////////////
   public static void Main( String [] arg )
   {
      ...
      GlgAirTrafficDemoExt.StandAlone = true;
      GlgAirTrafficDemoExt air_traffic_demo = new GlgAirTrafficDemoExt();      
   ...
   public void ProcessArgs( String [] arg )
   {
      ...
      if( num_arg != 0 )
      {
         for( int skip = 0; skip < num_arg; ++skip )
         {
            ...
            else if( arg[ skip ].equals( "-use_request" ) )
            {
               USE_REQUEST = true;
            }
            else if( arg[ skip ].equals( "-no_request" ) )
            {
               USE_REQUEST = false;
            }
            else if( arg[ skip ].equals( "-map_server_url" ) )
            {
               ...
               GlgAirTrafficDemoExt.SuppliedMapServerURL = arg[ skip ];
            }
            else if( arg[ skip ].equals( "-help" ) )
            {
               error( "Options: -num_planes <number> -speed <number> -ortho -map_server_url <URL> -sync -async -use_request -no_request\n" + 
                      "Defaults: -num_planes 100 -speed 0.005 -async -use_request", true );
            }
            else
            {
   ...
   public void ReadyCallback( GlgObject viewport )
   {
      ...
      */
      ...
      if( UseRequest )
        GISObject.SetScrollbarObserver( null, this );
   }   
   ...
   //////////////////////////////////////////////////////////////////////////
   public void UpdateObjectsOnMap( boolean using_request )
   {
      int i;
   ...
   //////////////////////////////////////////////////////////////////////////
   void SetupZoomLayers( GlgGISRequestData request_data )
   {
      String base_layers;
      boolean city_labels;
      ...
      GlgPoint extent = GetExtentDegrees( request_data );
      ...
      {
         String layers = CreateLayerString( base_layers, city_labels );
         ...
         {
            NewLayers = layers;
            NewFlags |= GlgObject.GIS_REQUEST_LAYERS;
         }
   ...
   ////////////////////////////////////////////////////////////////////////
   String CreateLayerString( String base_layers, boolean city_labels )
   {
      String layers = base_layers;
   ...
   void SetupZoomIcons()
   {
      GlgPoint extent = GetExtentDegrees( null );
   ...
   //////////////////////////////////////////////////////////////////////////
   void ChangeProjection( GlgGISRequestData request_data )
   {
      int projection = GetMapProjection( request_data );

      GlgPoint extent = GetExtentDegrees( request_data );
      if( extent.x < 30.0 && extent.y < 30.0 )
      {
         if( projection == GlgObject.RECTANGULAR_PROJECTION )
            return;    // Already rect, no change.

         // Change to the rectangular projection, extent is already in degrees.
         projection = GlgObject.RECTANGULAR_PROJECTION;
      }
      else
      {
         if( projection == GlgObject.ORTHOGRAPHIC_PROJECTION )
            return;    // Already ortho, no change

         // Change to the orthographic projection
         projection = GlgObject.ORTHOGRAPHIC_PROJECTION;

         // Change extent to meters
         extent.x = extent.x / 90.0 * GlgObject.EQUATOR_RADIUS;
         extent.y = extent.y / 90.0 * GlgObject.POLAR_RADIUS;
      }
      ...
      else
      {
         ...
         NewFlags |= ( GlgObject.GIS_REQUEST_PROJECTION | 
                       GlgObject.GIS_REQUEST_EXTENT );
      }
   }
      ...
      */
      ...
      else if( action.equals( "Zoom" ) )
      {
         if( subaction.equals( "End" ) )
         {
            ...
            */
            if( ( ASYNC_MAP || UseRequest ) &&
                GISObject.GetGISRequestInfo( null ) != null )
              SetMapLoading( true, "Loading map..." );
      ...
      else if( action.equals( "Pan" ) )
      {
         ...
         else if( subaction.equals( "Drag" ) )    // Dragging
         {
            ...
            */
            if( !UseRequest )
            {
               ...
               if( ASYNC_MAP )
                 SetMapLoading( true, "Loading map..." );
               SetStatus( "Dragging the map with mouse..." );
            }
            else
         ...
         {
            DraggingFromButton = false;
            if( UseRequest && !subaction.equals( "Abort" ) )
              SetMapLoading( true, "Loading map..." );
            else
       ...
       case MouseEvent.MOUSE_PRESSED:
         ...
         */
         ...
         {      
            ...
            // Pan the map
            NewCenter = new GlgPoint( lat_lon );
            NewFlags = GlgObject.GIS_REQUEST_CENTER;
      ...
      switch( type )
      {
       default:
         if( UseRequest )
         {
            ...
            */
            Map.RequestGISZoom( null, type, value, this );
            RequestType = type;
         }
         else
         {
       ...
       case 'n':
         ...
         NewMapProjection = GlgObject.ORTHOGRAPHIC_PROJECTION;
         NewCenter = new GlgPoint( InitCenter );
         NewExtent = new GlgPoint( InitExtent );
         NewFlags = ( GlgObject.GIS_REQUEST_PROJECTION | 
       ...
       case 't':
         // Start ZoomTo
         if( UseRequest )
           Map.RequestGISZoom( null, 't', 0.0, this );
         else
       ...
       case 's':
         ...
         if( UseRequest )
           Map.RequestGISZoom( null, 's', 0.0, this );
         else
   ...
   //////////////////////////////////////////////////////////////////////////
   void RequestNewMap( boolean update_objects, boolean call_update )
   {
      if( NewFlags == 0 )
        return;   // No change
      ...
      if( UseRequest )
      {
         ...
         */
         GISObject.RequestGISMap( null, NewExtent.x, NewExtent.y, 
                                  ...
                                  NewLayers, NewFlags, this );
         NewFlags = 0;
      }
      else   /* Set GIS object parameters directly and let it load the map. */
      {
         if( ( NewFlags & GlgObject.GIS_REQUEST_PROJECTION ) != 0 )
           GISObject.SetDResource( "GISProjection", (double) NewMapProjection );
         if( ( NewFlags & GlgObject.GIS_REQUEST_EXTENT ) != 0 )
           GISObject.SetGResource( "GISExtent", NewExtent );
         if( ( NewFlags & GlgObject.GIS_REQUEST_CENTER ) != 0 )
           GISObject.SetGResource( "GISCenter", NewCenter );
         if( ( NewFlags & GlgObject.GIS_REQUEST_LAYERS ) != 0 )
   ...
   //////////////////////////////////////////////////////////////////////////
   void CheckScrollLimits( GlgGISRequestData request_data )
   {
      ...
      */
      if( request_data == null && ( adjust_x || adjust_y ) )
  ...
  ////////////////////////////////////////////////////////////////////////
   ...
   ////////////////////////////////////////////////////////////////////////
   int GetMapProjection( GlgGISRequestData request_data )
   {   
      if( request_data != null )
   ...
   ////////////////////////////////////////////////////////////////////////
   GlgPoint GetMapExtent( GlgGISRequestData request_data )
   {   
      if( request_data != null )
   ...
   ////////////////////////////////////////////////////////////////////////
   GlgPoint GetMapCenter( GlgGISRequestData request_data )
   {   
      if( request_data != null )
   ...
   ////////////////////////////////////////////////////////////////////////
   GlgPoint GetExtentDegrees( GlgGISRequestData request_data )
   {   
      ...
      {
         if( request_data != null )
           extent = new GlgPoint( extent );  // Create a copy: will be modified.
   ...
   void UpdatePlanes()
   {
      ...
      /* If UseRequest=true, the map loading message will be erased 
         when the request is installed.
      */
      if( InitialLoad ||
          !UseRequest && !MapLoaded &&
          GISObject.GetDResource( "ImageLoaded" ).intValue() != 0 )
      {         
         SetMapLoading( false, "" );
         InitialLoad = false;
      }
      ...
      if( ( flags & GlgObject.GIS_REQUEST_EXTENT ) != 0 )
      {
         ...
         */
         ...
         if( ( flags & GlgObject.GIS_REQUEST_CENTER ) != 0 ||
             ( flags & GlgObject.GIS_REQUEST_PROJECTION ) != 0 )
   ...
   void ZoomToFlorida()
   {
      ...
      NewExtent = new GlgPoint( 1169530.0, 1169530.0, 0.0 );
      NewCenter = new GlgPoint( -82.8239, 28.9382, 0.0 );
      NewFlags = ( GlgObject.GIS_REQUEST_EXTENT |
                   GlgObject.GIS_REQUEST_CENTER );
   ...
   {
      GlgAirTrafficDemoExt bean;
      int stage;

      public ZoomPerformer( GlgAirTrafficDemoExt bean_p )
      {
         bean = bean_p;
         stage = 0;
      }
