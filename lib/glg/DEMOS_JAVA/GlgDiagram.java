...
//
// The demo uses GLG Installable Interface Handlers API for handling
// editor-style user interaction. Refer to the GLG Installable Interface
// Handlers section in the GLG Programming Reference Manual for an overview
// of the interface handlers functionality. 
// Refer to the GLG Java Online Documentation for documentation of the Interface
...
public class GlgDiagram extends GlgJBean implements ActionListener
{
   ...
   static EditPropertiesIH EditPropertiesIHInstance = null;
      ...
      public void EntryPoint( GlgObject ih, GlgCallEvent call_event )
      {
         ...
         */
         switch( IHGetType( call_event ) )
         {
          ...
          case MESSAGE_EVENT:
            /* Retrieve the token from the event and handle known tokens as 
               needed.
            */
            ...
            switch( token )
            {
             ...
             case IH_ICON_SELECTED:
               /* Retrieve selected icon parameters. 
                  ...
                  These parameters are global and are assigned in the input 
             ...
             case IH_MOUSE_PRESSED:
               ...
               */
               ...
               /* All tokens that originate from the TraceCB require an explicit
                  update. For tokens originating from InputCallback, update 
                  is done at the end of InputCallback.
               */            
               Update();
               break;
             ...
             default: 
               ...
               */
               ...
               // Create an instance of the EditPropertiesIH handler just once.
               if( EditPropertiesIHInstance == null )
                 EditPropertiesIHInstance = new EditPropertiesIH( diagram );
               ...
               */
               ...
               /* Install EditPropertiesIH handler, start it and invoke it with 
                  a given token.
               */ 
               IHPassToken( EditPropertiesIHInstance, token, false );
         ...
         switch( IHGetType( call_event ) )
         {
          ...
          case MESSAGE_EVENT:
            ...
            switch( token )
            {
             ...
             default:
               /* Unrecognized token: uninstall the current handler and invoke 
                  the parent handler, passing call_event to it.
               */
   ...
   // IH_MOUSE_MOVED tokens by the IHCallCurrIHWithToken method
   ...
   class AddNodeIH implements GlgIHHandlerInterface
   {
      ...
      public void EntryPoint( GlgObject ih, GlgCallEvent call_event )
      {
         ...
         switch( IHGetType( call_event ) )
         {
          ...
          case CLEANUP_EVENT:  // Triggered when handler is uninstalled.
            TraceMouseMove = false;
            SetRadioBox( SELECT_BUTTON_NAME );   // Highlight Select button
            SetPrompt( "" );
            Update();
      ...
      public void EntryPoint( GlgObject ih, GlgCallEvent call_event )
      {
         ...
         switch( IHGetType( call_event ) )
         {
          ...
          case HI_RESETUP_EVENT:
            /* This event type is triggered by the IHResetup call in this 
               handler, which is used to reset the handler to the inital state 
               to continue creating new links of this type when
               StickyCreateMode=True.
            */
          ...
          case MESSAGE_EVENT:
            ...
            switch( token )
            {
             ...
             case IH_DIALOG_CONFIRM_DISCARD:
               ...
               */
               IHInstall( new ConfirmIH( diagram ) );
               IHSetSParameter( IH_NEW, "title", "Confirmation Dialog" );
   ...
   {     
      ...
      {
         position = new GlgPoint( node_data.position );
