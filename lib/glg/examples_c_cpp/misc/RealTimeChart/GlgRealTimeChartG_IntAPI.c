...
void InitChartAfterH()
{
   ...
   /* Store object IDs for indivdual plot objects. */
   for( i=0; i<NUM_PLOTS; ++i )
   {
      res_name = GlgCreateIndexedName( "Plots/Plot#%", i );
      Plots[i] = GlgGetResourceObject( Chart, res_name );
      GlgFree( res_name );
   }
   ...
   switch( event_type )
   {
    ...
    case BUTTON_PRESS:
      ...
      /* Start dragging with the mouse on a mouse click. 
         ...
         direction of that axis. If the user clicked on the chart area,
